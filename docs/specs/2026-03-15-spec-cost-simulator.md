...
| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| 모델 선택 | 멀티셀렉트 | - | DB에서 검색, 최대 4개 |
| 일 평균 요청 건수 | number | 1,000 | |
| 평균 Input 토큰 | number | 500 | 요청당 |
| 평균 Output 토큰 | number | 300 | 요청당 |
...
cost = (realtime_input * inputPrice
      ...
      + batch_cached_in * batchInputPrice * cacheDiscount
      + realtime_output * outputPrice
      + batch_output * batchOutputPrice) / 1_000_000
```

OSS 모델(pricingType: "self-hosted")은 API 비용 = $0로 표시하고, "셀프호스팅 비용은 손익분기점 탭에서 확인" 안내.
...
- "BVA 성능 분석 보기" 링크 (선택 모델 slug로 BVA 연결)
...
선택한 모델 중 OSS 모델(`pricingType: "self-hosted"`)이 1개 이상 있을 때 활성화. 없으면 "OSS 모델을 선택하면 셀프호스팅 비용을 비교할 수 있습니다" 안내.
...
```
routedCost = sum(model.apiCost * model.trafficRatio for each model)

// 기준: 가장 비싼 모델로 100% 처리 시 비용
baselineCost = max(model.apiCost for each model)
...
interface IModelPricing {
  ...
  // 신규
  readonly cachedInputPer1m: number | null   // 캐싱 적용 input 단가
  readonly batchInputPer1m: number | null    // 배치 input 단가
  readonly batchOutputPer1m: number | null   // 배치 output 단가
}
```

시드 데이터에 주요 프로바이더 캐싱/배치 가격을 추가한다. null이면 업계 평균 할인율(50%)로 기본 단가에서 계산한다.
...
interface IGpu {
  // ... 기존 필드
  readonly hourlyRateUsd: number | null  // 클라우드 참고 시세 ($/hr)
}
```

Lambda Labs / RunPod 기준 대표 시세를 시드에 추가한다.
...
- `GET /api/gpus` — 기존 GPU 목록 API 활용 (hourlyRateUsd 추가)
